...
void CHud::RenderSpectatorHud()
{
	...
	char help[128];
	char version[128];
	str_format(version, sizeof(version), "%s", "v7.7");
	str_format(prMulti, sizeof(prMulti), "%s%d", GameClient()->m_MultiViewPersonalZoom > 0 ? "+" : "", GameClient()->m_MultiViewPersonalZoom);
	str_format(MultiView, sizeof(MultiView), "%s (zoom: %.1f (%s), cdist: %.0f, pdist: %.0f, cvel: %.3f, pvel: %.0f, frz: %i)",
		version,
		(std::log(m_pClient->m_Camera.m_Zoom) / std::log(0.866025f)) + 10,
		...
		GameClient()->m_MultiViewIgnoring);
	str_format(help, sizeof(help), "%s", m_pClient->m_Snap.m_SpecInfo.m_SpectatorID != SPEC_FREEVIEW ? (GameClient()->m_MultiViewActivated ? MultiView : m_pClient->m_aClients[m_pClient->m_Snap.m_SpecInfo.m_SpectatorID].m_aName) : (GameClient()->m_MultiViewActivated ? MultiView : Localize("Free-View")));
	...
	TextRender()->Text(m_Width - (g_Config.m_ClMultiViewDebug ? 314.0f : 174.0f), m_Height - 15.0f + (15.f - 8.f) / 2.f, 8.0f, aBuf, -1.0f);
}
