...
{
	// draw the box
	RenderTools()->DrawRect(m_Width - (g_Config.m_ClMultiViewDebug ? 320.0f : 180.0f), m_Height - 15.0f, 320.0f, 15.0f, ColorRGBA(0.0f, 0.0f, 0.0f, 0.4f), CUI::CORNER_TL, 5.0f);
	...
	str_format(MultiView, sizeof(MultiView), "%s (zoom: %.1f%s (%s), cdist: %.0f, pdist: %.0f, cvel: %.3f, pvel: %.0f, ids: %s)",
		...
		GameClient()->m_MultiViewIsWidthMode ? "w" : "h",
