...
void CGameClient::HandleMultiView()
{
	...
	if(vec2(posx, posy).operator!=(vec2(0, 0)))
		m_Snap.m_SpecInfo.m_Position = m_MultiViewOldPos + ((vec2(posx, posy) - m_MultiViewOldPos) * multiplier);
	else
...
float CGameClient::MultiplierStuff(vec2 camerapos)
{
	...
	m_MultiViewMultiplier = MapValue(maxCameraDist, minCameraDist, maxVel, minVel, tmp);

	return clamp(m_MultiViewMultiplier, 0.0f, 1.0f);
}
...
float CGameClient::ZoomStuff(vec2 minpos, vec2 maxpos)
{
	...
	{
		if(!m_MultiViewIsWidthMode)
		{
			if(m_MultiViewLastSwitchTick > time_get())
			{
				width = false;
			}
			else
			{
				m_MultiViewIsWidthMode = true;
				m_MultiViewLastSwitchTick = time_get() + time_freq() * 3;
				width = true;
			}
		}
		else
	...
	float zoom = MapValue(maxPlayerDistance, minPlayerDistance, maxZoom, minZoom, m_MultiViewPlayerDistance);
